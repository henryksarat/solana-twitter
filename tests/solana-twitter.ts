...
describe("solana-twitter", () => {
  ...
  // });
   ...
   it('can send a new toast', async () => {
        // Call the "SendTweet" instruction.
        const toast = anchor.web3.Keypair.generate();
        await program.rpc.messageMe('whats up?', {
            accounts: {
                toast: toast.publicKey,
        ...
        // // Fetch the account details of the created toast.
        const toastAccount = await program.account.toast.fetch(toast.publicKey);
        console.log(toastAccount);
        
        assert.equal(toastAccount.author.toBase58(), program.provider.wallet.publicKey.toBase58());
        assert.equal(toastAccount.message, 'whats up?');
        assert.ok(toastAccount.timestamp);
    });
